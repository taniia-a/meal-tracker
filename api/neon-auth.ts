...
function firstPartyCookie(cookie: string) {
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      const path = incomingUrl.searchParams.get('path') ?? incomingUrl.pathname.replace(/^\/api\/neon-auth\/?/, '');
      ...
      setCookies.forEach((cookie) => responseHeaders.append('set-cookie', firstPartyCookie(cookie)));

      return new Response(upstream.body, { status: upstream.status, headers: responseHeaders });
    } catch (error) {
      console.error('Neon Auth proxy error', error);
      return Response.json({ error: 'Não foi possível contactar o Neon Auth.' }, { status: 502 });
    }
  },
};
