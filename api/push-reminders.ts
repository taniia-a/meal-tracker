...
export default async function handler(request: Request) {
  ...
  try {
    ...
    for (const subscription of rows) {
      ...
      if (subscription.reminders.meals && timeMatches(subscription.reminders.mealsTime, now.hour, now.minute)) jobs.push({ kind: 'meals', title: 'Meal Tracker', body: 'Lembra-te de registar as tuas refeições de hoje.', slot: `${now.day}-meals` });
