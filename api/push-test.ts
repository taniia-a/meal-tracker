...
export default async function handler(request: Request) {
  ...
  if (!process.env.DATABASE_URL || !process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY || !process.env.VAPID_SUBJECT) return Response.json({ error: 'As notificações push ainda não estão configuradas no servidor.' }, { status: 503 });
  try {
