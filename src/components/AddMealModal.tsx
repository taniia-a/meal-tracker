  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-ink/40 p-5 backdrop-blur-sm">
      <form onSubmit={submit} className="card w-full max-w-md p-6">
        ...
        <label className="mt-6 block text-sm font-semibold">{t('Dia da refeição')}<span className="relative mt-2 block w-48 max-w-full"><CalendarDays className="pointer-events-none absolute left-4 top-3.5 text-stone-400" size={18} /><input className="input compact-date-input !pl-12" type="date" required value={date} onChange={(e) => setDate(e.target.value)} /></span></label>
