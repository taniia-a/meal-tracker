...
const navigation = [
  ...
  { to: "/despensa", label: "Stock", icon: PackageOpen },
  { to: "/progresso", label: "Progresso", icon: ChartNoAxesCombined },
  { to: "/curiosidade", label: "Sabias que…?", icon: Lightbulb },
...
export default function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <header className="sticky top-0 z-30 border-b border-white/10 bg-cream/90 backdrop-blur-xl lg:hidden">
        <div className="relative flex h-16 items-center justify-center px-5">
          ...
          <button
            ...
            className="absolute left-5 rounded-xl p-2 hover:bg-white/5"
      ...
      <aside
        className={`${open ? "translate-x-0" : "-translate-x-full"} fixed inset-y-0 left-0 z-40 w-72 border-r border-white/10 bg-[#120e18] p-6 transition-transform lg:translate-x-0`}
      >
        <Brand />
        ...
        <div className="absolute inset-x-6 bottom-6 rounded-3xl bg-ink p-4 text-white">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex rounded-xl bg-white/5 p-1">
              <button
                ...
                className={`rounded-lg px-2.5 py-1 text-xs font-bold ${i18n.language === "pt" ? "bg-leaf-600 text-white" : "text-stone-400"}`}
              >
                PT
              </button>
              <button
                onClick={() => changeLanguage("en")}
                className={`rounded-lg px-2.5 py-1 text-xs font-bold ${i18n.language === "en" ? "bg-leaf-600 text-white" : "text-stone-400"}`}
              >
                EN
              </button>
            </div>
          ...
          <button
            ...
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-white/5 px-3 py-2 text-sm font-bold text-stone-300 transition hover:bg-white/10 hover:text-white"
          >
      ...
      {showBackToTop && (
        <button
          ...
          className="fixed bottom-5 right-5 z-30 grid h-12 w-12 place-items-center rounded-2xl bg-leaf-600 text-white shadow-lg shadow-leaf-600/30 transition hover:bg-leaf-700 sm:bottom-7 sm:right-7"
...
function Brand({
  ...
  return (
    ...
    >
      {!hideIcon && (
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-leaf-600 text-white">
          <ChefHat size={24} />
        </div>
      )}
      <div>
