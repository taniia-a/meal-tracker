...
export default function DiaryPage() {
  ...
  return <div className="mx-auto max-w-7xl">
    <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="font-semibold text-leaf-600">{t('Histórico')}</p><h1 className="mt-1 text-3xl font-extrabold sm:text-4xl">{t('Diário de refeições')}</h1><p className="mt-2 text-stone-500">{t('Consulta e gere tudo o que registaste.')}</p></div><div className="flex flex-col gap-3 sm:flex-row"><div className="flex rounded-2xl border border-white/10 bg-white/5 p-1"><button onClick={() => setView('day')} className={`flex-1 rounded-xl px-4 py-2 text-sm font-bold ${view === 'day' ? 'bg-leaf-600 text-white' : 'text-stone-400'}`}>{t('Dia')}</button><button onClick={() => setView('week')} className={`flex-1 rounded-xl px-4 py-2 text-sm font-bold ${view === 'week' ? 'bg-leaf-600 text-white' : 'text-stone-400'}`}>{t('Semana')}</button></div>{view === 'day' && <label className="relative"><CalendarDays className="absolute left-4 top-3.5 text-stone-400" size={19} /><input className="input !pl-12" type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label>}</div></div>
    ...
    </> : <WeeklyView week={week} entries={entries} locale={locale} today={today} onPrevious={() => moveWeek(-1)} onNext={() => moveWeek(1)} onCurrent={() => setDate(today)} onOpenDay={openDay} />}
...
function WeeklyView({ week, entries, locale, today: currentDay, onPrevious, onNext, onCurrent, onOpenDay }: { week: string[]; entries: MealEntry[]; locale: string; today: string; onPrevious: () => void; onNext: () => void; onCurrent: () => void; onOpenDay: (day: string) => void }) {
  const { t, i18n } = useTranslation();
  ...
  return <section className="mt-8"><div className="flex flex-wrap items-center justify-between gap-3"><button onClick={onCurrent} className="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold hover:bg-white/5">{t('Semana atual')}</button><div className="flex items-center gap-3"><button onClick={onPrevious} className="rounded-xl border border-white/10 p-2 hover:bg-white/5" aria-label={t('Semana anterior')}><ChevronLeft /></button><h2 className="min-w-44 text-center font-bold capitalize">{range}</h2><button onClick={onNext} className="rounded-xl border border-white/10 p-2 hover:bg-white/5" aria-label={t('Semana seguinte')}><ChevronRight /></button></div></div>
