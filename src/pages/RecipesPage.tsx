...
export default function RecipesPage() {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="mt-8 flex flex-col gap-3"><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-4 top-3.5 text-stone-400" size={20} /><input className="input pl-12" placeholder={t('Ex.: frango, aveia, salmão...')} value={query} onChange={(event) => setQuery(event.target.value)} /></label><select className="input sm:w-56" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item} value={item}>{t(item)}</option>)}</select></div><div className="flex w-full rounded-2xl border border-white/10 bg-white/5 p-1 sm:w-fit">{(['all', 'mine', 'public'] as Scope[]).map((item) => <button key={item} onClick={() => setScope(item)} className={`flex-1 rounded-xl px-4 py-2 text-sm font-bold sm:flex-none ${scope === item ? 'bg-leaf-600 text-white' : 'text-stone-400 hover:text-white'}`}>{t(item === 'all' ? 'Todas' : item === 'mine' ? 'As minhas' : 'Públicas')}</button>)}</div></div>
