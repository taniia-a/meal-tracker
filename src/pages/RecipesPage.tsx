...
export default function RecipesPage() {
  ...
  const [taste, setTaste] = useState<(typeof tastes)[number]>("Todas");
      ...
      recipes
        .filter((recipe) => {
          ...
          const matchesText = terms.every((term) =>
            searchable.some((item) => item.includes(term)),
          );
          const matchesCategory =
            category === "Todas" || recipe.category === category;
          const matchesTaste = taste === "Todas" || recipe.taste === taste;
          const matchesScope =
            scope === "all" ||
            (scope === "mine"
          ...
          return matchesText && matchesCategory && matchesTaste && matchesScope;
        })
        .sort((a, b) =>
          sort === "recent"
            ? 0
        ...
        .map((recipe) => ({
          ...
          ingredients: recipeIngredients(recipe, i18n.language),
        })),
    [
      recipes,
      query,
  ...
  const duplicate = async (recipe: Recipe) => {
    ...
    const input: RecipeInput = {
      ...
      nameEn: source.nameEn
        ? `${source.nameEn}${source.nameEn.endsWith("(copy)") ? "" : " (copy)"}`
        : "",
      ...
      ingredientQuantities: [...source.ingredientQuantities],
      ingredientUnits: [...source.ingredientUnits],
      ingredientOptional: [...source.ingredientOptional],
    ...
    try {
      ...
      setSuccess(t("Receita duplicada com sucesso. A cópia é privada."));
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : t("Não foi possível duplicar a receita."),
      );
    } finally {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          <p className="font-semibold text-leaf-600">{t("Biblioteca")}</p>
          <h1 className="mt-1 text-3xl font-extrabold sm:text-4xl">
            {t("Encontra a próxima refeição")}
          </h1>
      ...
      <aside className="mt-7 rounded-3xl border border-leaf-500/20 bg-leaf-500/10 p-5 text-sm leading-relaxed text-stone-300">
        <p className="font-bold text-leaf-700">{t("Sobre as receitas")}</p>
      ...
      <div className="mt-8 flex flex-col gap-3">
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_14rem_10rem]">
          ...
          >
            ...
            <option value="prep">{t("Preparação mais rápida")}</option>
            <option value="calories-desc">{t("Mais calorias")}</option>
            <option value="calories-asc">{t("Menos calorias")}</option>
            <option value="protein-desc">{t("Mais proteína")}</option>
            <option value="protein-asc">{t("Menos proteína")}</option>
          </select>
          <select
            className="input"
            value={category}
          ...
          <select
            className="input"
            value={taste}
            onChange={(event) => setTaste(event.target.value as (typeof tastes)[number])}
            aria-label={t("Tipo")}
          >
            {tastes.map((item) => (
              <option key={item} value={item}>
                {item === "Todas" ? t(item) : t(item)}
              </option>
            ))}
          </select>
        </div>
        <div className="flex w-full rounded-2xl border border-white/10 bg-white/5 p-1 sm:w-fit">
          {(["all", "mine", "public", "favorites"] as Scope[]).map((item) => (
            <button
              key={item}
              onClick={() => setScope(item)}
              className={`flex-1 rounded-xl px-4 py-2 text-sm font-bold sm:flex-none ${scope === item ? "bg-leaf-600 text-white" : "text-stone-400 hover:text-white"}`}
            >
              {t(
                item === "all"
                  ? "Todas"
      ...
      {success && (
        <p
          role="status"
          className="mt-5 flex items-center gap-2 rounded-2xl bg-emerald-500/10 p-4 text-sm text-emerald-300"
        >
      ...
      {isRecipesLoading && (
        <div className="mt-10 text-center text-stone-400">
          <Loader2 className="mx-auto animate-spin text-leaf-500" />
          <p className="mt-3 text-sm">{t("A carregar receitas...")}</p>
        </div>
      )}
      {!isRecipesLoading && filtered.length === 0 && (
        <div className="card mt-5 p-10 text-center">
          <Utensils className="mx-auto text-leaf-500" size={38} />
          <h2 className="mt-4 text-xl font-bold">
            {t("Ainda não existem receitas")}
          </h2>
          <p className="mt-2 text-sm text-stone-400">
            {t("Experimenta alterar os filtros ou criar uma receita.")}
          </p>
        </div>
      )}
      ...
      <section className="mt-4 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {visibleRecipes.map((recipe) => {
          ...
          const translated = Boolean(
            recipe.nameEn.trim() &&
            (!recipe.instructions.trim() || recipe.instructionsEn.trim()) &&
            recipe.ingredientsEn.length === recipe.ingredients.length &&
            recipe.ingredientsEn.every((item) => item.trim()),
          );
          ...
          return (
            ...
            >
              <div
                className={`relative flex h-48 items-center justify-center overflow-hidden bg-gradient-to-br sm:h-52 ${recipe.imageColor}`}
              >
                <Link
                  to={`/receitas/${recipe.id}`}
                  className="absolute inset-0"
                  aria-label={t("Ver receita {{name}}", { name: recipe.name })}
                >
                  {recipe.imageUrl ? (
                    <img
                      src={recipe.imageUrl}
                    ...
                    <span className="flex h-full items-center justify-center">
                      <Utensils size={45} className="text-white/80" />
                    </span>
                  )}
                </Link>
                <div className="absolute right-3 top-3 flex gap-2">
                  <button
                    onClick={() => {
                      setError("");
                      toggleFavorite(recipe.id).catch(() =>
                        setError(t("Não foi possível atualizar os favoritos.")),
                      );
                    }}
                    className={`rounded-xl bg-black/40 p-2 backdrop-blur hover:bg-black/60 ${favorite ? "text-rose-300" : "text-white"}`}
                    aria-label={t(
                      favorite
                  ...
                  <button
                    ...
                    className="rounded-xl bg-black/40 p-2 text-white backdrop-blur hover:bg-leaf-600 disabled:opacity-50"
                    aria-label={t("Duplicar receita")}
                    title={t("Duplicar receita")}
                  >
                    <Copy size={16} />
                  </button>
                  {owner && (
                    <>
                      <Link
                        ...
                        className="rounded-xl bg-black/40 p-2 text-white backdrop-blur hover:bg-black/60"
                        aria-label={t("Editar receita")}
                      >
                        <Pencil size={16} />
                      </Link>
                      <button
                        onClick={() => remove(recipe)}
                        className="rounded-xl bg-black/40 p-2 text-white backdrop-blur hover:bg-rose-500/70"
                        aria-label={t("Apagar receita")}
                      >
                        <Trash2 size={16} />
                      </button>
                    </>
              ...
              <div className="p-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span
                    className={`flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-bold ${translated ? "bg-emerald-500/10 text-emerald-300" : "bg-amber-500/10 text-amber-300"}`}
                    title={t(
                      translated
                        ? "Tradução inglesa completa"
                        : "Tradução inglesa em falta",
                    )}
                  >
                ...
                <Link
                  to={`/receitas/${recipe.id}`}
                  className="mt-4 block text-lg font-bold hover:text-leaf-700"
                >
                  {recipe.name}
                </Link>
                <div className="mt-5 grid grid-cols-4 gap-2 text-center">
                  <Macro value={recipe.calories} label="kcal" />
                  <Macro value={recipe.protein} label={t("prot.")} />
                  <Macro value={recipe.carbs} label={t("hidr.")} />
                  <Macro value={recipe.fat} label={t("gord.")} />
                </div>
                <div className="mt-5 grid grid-cols-2 gap-2">
                  <Link
                    to={`/receitas/${recipe.id}`}
                    className="flex items-center justify-center gap-2 rounded-2xl border border-white/10 px-4 py-3 font-bold hover:bg-white/5"
                  >
                    <Eye size={18} /> {t("Ver")}
                  </Link>
                  <button
                    onClick={() => setSelected(recipe)}
                    className="flex items-center justify-center gap-2 rounded-2xl bg-ink px-4 py-3 font-bold text-white hover:bg-leaf-700"
                  >
                    <Plus size={18} /> {t("Registar")}
                  </button>
                </div>
              </div>
            </article>
