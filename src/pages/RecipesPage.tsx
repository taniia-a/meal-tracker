import { CheckCircle2, Clock, Copy, Eye, Heart, Languages, Loader2, Pencil, Plus, Search, Trash2, Utensils } from 'lucide-react';
...
type Sort = 'recent' | 'prep' | 'calories-desc' | 'calories-asc' | 'protein-desc' | 'protein-asc';

export default function RecipesPage() {
  const { recipes, favoriteRecipeIds, isRecipesLoading, profile, deleteRecipe, saveRecipe, toggleFavorite } = useMeals();
  ...
  const filtered = useMemo(() => recipes.filter((recipe) => {
    const terms = query.trim().toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').split(/\s+/).filter(Boolean);
    const searchable = [recipe.name, recipe.nameEn, ...recipe.ingredients, ...recipe.ingredientsEn].map((item) => item.toLocaleLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''));
    const matchesText = terms.every((term) => searchable.some((item) => item.includes(term)));
    const matchesCategory = category === 'Todas' || recipe.category === category;
    const matchesScope = scope === 'all' || (scope === 'mine' ? recipe.ownerId === profile.userId : scope === 'public' ? recipe.isPublic : favoriteRecipeIds.includes(recipe.id));
    return matchesText && matchesCategory && matchesScope;
  }).sort((a, b) => sort === 'recent' ? 0 : sort === 'prep' ? a.prepMinutes - b.prepMinutes : sort === 'calories-desc' ? b.calories - a.calories : sort === 'calories-asc' ? a.calories - b.calories : sort === 'protein-desc' ? b.protein - a.protein : a.protein - b.protein).map((recipe) => ({ ...recipe, name: recipeName(recipe, i18n.language), ingredients: recipeIngredients(recipe, i18n.language) })), [recipes, query, category, scope, sort, profile.userId, favoriteRecipeIds, i18n.language]);
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="mt-8 flex flex-col gap-3"><div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_14rem]"><label className="relative min-w-0"><Search className="absolute left-4 top-3.5 text-stone-400" size={20} /><input className="input !pl-12" placeholder={t('Ex.: frango, aveia, salmão...')} value={query} onChange={(event) => setQuery(event.target.value)} /></label><select className="input" value={sort} onChange={(event) => setSort(event.target.value as Sort)} aria-label={t('Ordenar por')}><option value="recent">{t('Mais recentes')}</option><option value="prep">{t('Preparação mais rápida')}</option><option value="calories-desc">{t('Mais calorias')}</option><option value="calories-asc">{t('Menos calorias')}</option><option value="protein-desc">{t('Mais proteína')}</option><option value="protein-asc">{t('Menos proteína')}</option></select><select className="input" value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item} value={item}>{t(item)}</option>)}</select></div><div className="flex w-full rounded-2xl border border-white/10 bg-white/5 p-1 sm:w-fit">{(['all', 'mine', 'public', 'favorites'] as Scope[]).map((item) => <button key={item} onClick={() => setScope(item)} className={`flex-1 rounded-xl px-4 py-2 text-sm font-bold sm:flex-none ${scope === item ? 'bg-leaf-600 text-white' : 'text-stone-400 hover:text-white'}`}>{t(item === 'all' ? 'Todas' : item === 'mine' ? 'As minhas' : item === 'public' ? 'Públicas' : 'Favoritas')}</button>)}</div></div>
