...
export default function RecipesPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="font-semibold text-leaf-600">Biblioteca</p><h1 className="mt-1 text-3xl font-extrabold sm:text-4xl">Encontra a próxima refeição</h1><p className="mt-2 text-stone-500">Pesquisa pelo nome da receita ou por um ingrediente.</p></div><Link to="/receitas/nova" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-leaf-600 px-5 py-3 font-bold text-white"><Plus size={18} /> Nova receita</Link></div>
      ...
      {isRecipesLoading && <div className="mt-10 text-center text-stone-400"><Loader2 className="mx-auto animate-spin text-leaf-500" /><p className="mt-3 text-sm">A carregar receitas...</p></div>}
      {!isRecipesLoading && filtered.length === 0 && <div className="card mt-5 p-10 text-center"><Utensils className="mx-auto text-leaf-500" size={38} /><h2 className="mt-4 text-xl font-bold">Ainda não existem receitas</h2><p className="mt-2 text-sm text-stone-400">Cria a primeira receita para começares.</p></div>}
