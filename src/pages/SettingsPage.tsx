...
function ReminderSettingsPanel() {
  ...
  const row = (key: 'meals' | 'weight', timeKey: 'mealsTime' | 'weightTime', label: string, description: string) => <div className="border-t border-white/10 py-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-bold">{t(label)}</p><p className="mt-1 text-sm text-stone-400">{t(description)}</p></div>{switchControl(key)}</div><div className="mt-5 w-full overflow-hidden rounded-2xl"><input className="input !w-full !min-w-0" type="time" disabled={!settings[key]} value={settings[timeKey]} onChange={(event) => time(timeKey, event.target.value)} /></div></div>;
  const hasActiveReminders = settings.meals || settings.water || settings.weight;
  return <section className="card mt-6 p-7"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-xl font-bold">{t('Lembretes')}</h2><p className="mt-1 text-sm text-stone-400">{t('Escolhe quando queres ser lembrada de acompanhar o teu dia.')}</p></div><button type="button" onClick={() => void enableNotifications()} className="rounded-2xl border border-leaf-500/40 px-4 py-2 text-sm font-bold text-leaf-300">{permission === 'granted' && hasActiveReminders ? t('Desativar lembretes') : t('Ativar notificações')}</button></div><div className="mt-5">{row('meals', 'mealsTime', 'Refeições', 'Lembra-te de registar uma refeição caso ainda não o tenhas feito.')}<div className="border-t border-white/10 py-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-bold">{t('Água')}</p><p className="mt-1 text-sm text-stone-400">{t('Recebe um lembrete de hora a hora entre as 08:00 e as 22:00, até atingires o objetivo.')}</p></div>{switchControl('water')}</div></div>{row('weight', 'weightTime', 'Peso', 'Recebe um lembrete semanal ao domingo para registar o peso.')}</div><p className="mt-2 text-xs text-stone-500">{t('Os lembretes funcionam enquanto o site estiver aberto neste dispositivo.')}</p>{feedback && <p role="status" className="mt-3 text-sm font-semibold text-leaf-600">{feedback}</p>}</section>;
}
