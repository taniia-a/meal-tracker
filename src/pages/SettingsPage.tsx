...
function ReminderSettingsPanel() {
  ...
  const enableNotifications = async () => {
    if (permission === 'granted') {
      const hasActive = settings.meals || settings.water || settings.weight || settings.stockExpiry;
      const next = { ...settings, meals: !hasActive, water: !hasActive, weight: !hasActive, stockExpiry: !hasActive };
      update(next);
    ...
    if (result === 'granted') {
      const next = { ...settings, meals: true, water: true, weight: true, stockExpiry: true };
  ...
  const toggle = (key: 'meals' | 'water' | 'weight' | 'stockExpiry') => update({ ...settings, [key]: !settings[key] });
  const switchControl = (key: 'meals' | 'water' | 'weight' | 'stockExpiry') => <button type="button" role="switch" aria-checked={settings[key]} onClick={() => toggle(key)} className={`h-7 w-12 shrink-0 rounded-full p-1 transition ${settings[key] ? 'bg-leaf-600' : 'bg-white/10'}`}><span className={`block h-5 w-5 rounded-full bg-white transition ${settings[key] ? 'translate-x-5' : ''}`} /></button>;
  const row = (key: 'meals' | 'weight' | 'stockExpiry', label: string, description: string) => <div className="border-t border-white/10 py-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-bold">{t(label)}</p><p className="mt-1 text-sm text-stone-400">{t(description)}</p></div>{switchControl(key)}</div></div>;
  const hasActiveReminders = settings.meals || settings.water || settings.weight || settings.stockExpiry;
  return <section className="card mt-6 p-7"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><h2 className="text-xl font-bold">{t('Lembretes')}</h2><p className="mt-1 text-sm text-stone-400">{t('Escolhe se queres receber notificações para acompanhar o teu dia.')}</p></div><button type="button" onClick={() => void enableNotifications()} className="rounded-2xl border border-leaf-500/40 px-4 py-2 text-sm font-bold text-leaf-300">{permission === 'granted' && hasActiveReminders ? t('Desativar lembretes') : t('Ativar notificações')}</button></div><div className="mt-5">{row('meals', 'Refeições', 'Lembra-te às 21:00 de registar as tuas refeições caso ainda não o tenhas feito.')}<div className="border-t border-white/10 py-4"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="font-bold">{t('Água')}</p><p className="mt-1 text-sm text-stone-400">{t('Recebe um lembrete de hora a hora entre as 08:00 e as 22:00, até atingires o objetivo.')}</p></div>{switchControl('water')}</div></div>{row('weight', 'Peso', 'Recebe um lembrete semanal ao domingo às 08:00 para registar o peso.')}{row('stockExpiry', 'Validade do stock', 'Recebe um lembrete diário às 09:00 quando existirem produtos a terminar nos próximos 7 dias.')}</div>{feedback && <p role="status" className="mt-3 text-sm font-semibold text-leaf-600">{feedback}</p>}</section>;
}
